        ...
        {(response) => {
          return <>
            <div className="home-image-gallery-top">
              {response.metaobjects.nodes.slice(0, 2).map((image: any) => {
                return <img src={image.fields[0].reference.image.url} />
              })}
            </div>
            <div className="home-image-gallery-bottom">
              {response.metaobjects.nodes.slice(2, 5).map((image: any) => {
                return <img src={image.fields[0].reference.image.url} />
              })}
            </div>
          </>
